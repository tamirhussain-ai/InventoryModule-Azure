...
app.post("/make-server-5ec3cec0/auth/forgot-password", async (c) => {
  ...
  try {
    ...
    console.log('[msal-signin] Verifying MSAL token with Microsoft Graph');
    const graphRes = await fetch('https://graph.microsoft.com/v1.0/me', {
      headers: { 'Authorization': `Bearer ${msalToken}` },
    });

    if (!graphRes.ok) {
      console.log('[msal-signin] Graph verification failed:', graphRes.status);
    ...
    console.log('[msal-signin] Verified user:', email);
    ...
    if (!userProfile) {
      ...
      console.log('[msal-signin] Created user profile, role:', userProfile.role);
    }
...
// ========== MSAL BRIDGE ENDPOINT ==========
// Accepts an MSAL access token from Azure Entra ID,
